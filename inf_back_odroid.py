...
with depthai.Device(pipeline) as device:
    ...
    if LENS_FOCUS == -1:
        ctrl.setAutoFocusMode(depthai.RawCameraControl.AutoFocusMode.AUTO)
    else : 
    ...
    while True:
        ...
        if time.time() - time_init > STREAM_DURATION :
            if not frame_is_clear and frame_keep is not None:
                ...
                print('We used the backup')
            elif not frame_is_clear and frame_keep is  None:
                print('Warning: No clear frame was obtained')
                print(frame_keep)
                print(frame_is_clear)    
            break
