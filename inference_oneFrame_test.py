...
with depthai.Device(pipeline) as device:
    ...
    while True:
        ...
        if (time.time() - time_init > EARLY_STOP * STREAM_DURATION and detections_list_keep != []):
            break
        ...
        if time.time() - time_init > STREAM_DURATION:
            if frame_keep is None:
                ...
                frame_keep = frame
            break
timestamp = store_data(frame_keep, detections_list_keep, OUTPUT_PATH, IMG_QUALITY)

if args.show_output:
    box_the_frame(frame_keep, detections_list_keep)
    cv2.imshow('test', frame_keep)
...
print('Blur score: ' + str(blur_score(frame_keep)))
print('Frame is clear:' + str(is_clear(frame_keep, BLUR_THRESHOLD)))
print('Number of detections : ' + str(len(detections_list_keep)))
print('=======================================================================')
